...
func main() {
	...
	dashboardPassword, err := randomDashboardPassword(*suffix)
	if err != nil {
		fatalf("generate dashboard password: %v", err)
	}
...
func randomDashboardPassword(suffix string) (string, error) {
	var raw [4]byte
	if _, err := rand.Read(raw[:]); err != nil {
		return "", err
	}
	return fmt.Sprintf("Sandbox-%x-%s", raw[:], suffix), nil
}
