...
func (h *Handler) runDueSubscriptions(w http.ResponseWriter, r *http.Request) {
	...
	// A batch run can partially fail. Returning an error here would discard the
	// invoices that were actually collected, so report both: the collected
	// invoices and the subscriptions that failed. Full error detail is logged
	// by the service; only the failing subscription ids are returned.
...
// returned by RunDueSubscriptions. Only ids are surfaced; the underlying errors
// stay in the logs.
