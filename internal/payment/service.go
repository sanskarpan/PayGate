...
func (s *Service) Authorize(ctx context.Context, in AuthorizeInput) (CaptureResult, error) {
	...
	// Label metrics with the provider that actually handled the attempt.
	// Under routing or failover this differs from the wrapper gateway name, and
	// attributing to the wrapper points incident analysis at the wrong gateway.
	// decision.Provider falls back to provider when nothing routed, so direct
	// gateway flows are unchanged.
	effectiveProvider := decision.Provider
	if effectiveProvider == "" {
		effectiveProvider = provider
	}
	metrics.GatewayAuthorizationDuration.WithLabelValues(in.Method, effectiveProvider).Observe(time.Since(startedAt).Seconds())
	if err != nil {
		metrics.GatewayAuthorizationsTotal.WithLabelValues(in.Method, effectiveProvider, "error").Inc()
	...
	if result.RequiresAction {
		...
		metrics.GatewayAuthorizationsTotal.WithLabelValues(in.Method, effectiveProvider, "requires_action").Inc()
	...
	if !result.Success {
		metrics.GatewayAuthorizationsTotal.WithLabelValues(in.Method, effectiveProvider, "declined").Inc()
	...
	metrics.GatewayAuthorizationsTotal.WithLabelValues(in.Method, effectiveProvider, "authorized").Inc()
	return out, nil
}
