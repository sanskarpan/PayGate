...
func TestIntegrationReportingExportTaxProfileAndStatements(t *testing.T) {
	...
	auth, err := paymentSvc.Authorize(ctx, payment.AuthorizeInput{MerchantID: createdMerchant.ID, OrderID: createdOrder.ID, Amount: createdOrder.Amount, Currency: createdOrder.Currency, Method: "card"})
	if err != nil {
...
func TestIntegrationReconSourceImportAssignmentAndResolution(t *testing.T) {
	...
	authz, err := paymentSvc.Authorize(ctx, payment.AuthorizeInput{MerchantID: createdMerchant.ID, OrderID: createdOrder.ID, Amount: createdOrder.Amount, Currency: createdOrder.Currency, Method: "card"})
	if err != nil {
		t.Fatalf("authorize: %v", err)
	}
	...
	sttlSvc := settlement.NewService(settlement.NewPostgresRepository(db, nil))
...
func TestIntegrationPaymentMethodStatesForCardAndUPI(t *testing.T) {
	...
	cardAuth, err := paymentSvc.Authorize(ctx, payment.AuthorizeInput{MerchantID: createdMerchant.ID, OrderID: cardOrder.ID, Amount: cardOrder.Amount, Currency: cardOrder.Currency, Method: "card"})
	if err != nil {
